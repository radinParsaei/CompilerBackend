  ...
  public static class LeftShift extends ValueBase implements java.io.Serializable {
    ...
    public Object getData() {
      ...
      if (v1 instanceof CreateInstance) {
        boolean overloaded = false;
        for (Map.Entry<String, ProgramBase> entry : functions.entrySet()) {
          if (entry.getKey().matches("#C" + ((CreateInstance) v1).getClassName() + "#LeftShift:,((?!,).)+")) {
            overloaded = true;
            break;
          }
        }
        if (overloaded) {
          return new SyntaxTree.CallFunction("LeftShift", v2).fromInstance(v1).setAddInstanceName(true).getData();
        }
      }
      if (v2 instanceof CreateInstance) {
        boolean overloaded = false;
        for (Map.Entry<String, ProgramBase> entry : functions.entrySet()) {
          if (entry.getKey().matches("#C" + ((CreateInstance) v2).getClassName() + "#LeftShift:,((?!,).)+")) {
            overloaded = true;
            break;
          }
        }
        if (overloaded) {
          return new SyntaxTree.CallFunction("LeftShift", v1).fromInstance(v2).setAddInstanceName(true).getData();
        }
      }
      if (v1 instanceof CreateInstance) v1 = getTextFromInstance(v1);
      if (v2 instanceof CreateInstance) v2 = getTextFromInstance(v2);
  ...
  public static class RightShift extends ValueBase implements java.io.Serializable {
    ...
    public Object getData() {
      ...
      if (v1 instanceof CreateInstance) {
        boolean overloaded = false;
        for (Map.Entry<String, ProgramBase> entry : functions.entrySet()) {
          if (entry.getKey().matches("#C" + ((CreateInstance) v1).getClassName() + "#RightShift:,((?!,).)+")) {
            overloaded = true;
            break;
          }
        }
        if (overloaded) {
          return new SyntaxTree.CallFunction("RightShift", v2).fromInstance(v1).setAddInstanceName(true).getData();
        }
      }
      if (v2 instanceof CreateInstance) {
        boolean overloaded = false;
        for (Map.Entry<String, ProgramBase> entry : functions.entrySet()) {
          if (entry.getKey().matches("#C" + ((CreateInstance) v2).getClassName() + "#RightShift:,((?!,).)+")) {
            overloaded = true;
            break;
          }
        }
        if (overloaded) {
          return new SyntaxTree.CallFunction("RightShift", v1).fromInstance(v2).setAddInstanceName(true).getData();
        }
      }
      if (v1 instanceof CreateInstance) v1 = getTextFromInstance(v1);
      if (v2 instanceof CreateInstance) v2 = getTextFromInstance(v2);
