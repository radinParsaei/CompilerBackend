    ...
    for (ValueBase val : vals) {
      if (val instanceof SyntaxTree.Number) {
        ...
        output.append(putVals(((SyntaxTree.Append) val).getList()))
                ...
                .append("APPEND\n");
      } else if (val instanceof SyntaxTree.Insert) {
        output.append(putVals(((SyntaxTree.Insert) val).getList()))
                .append(putVals(((SyntaxTree.Insert) val).getValue()))
                .append(putVals(((SyntaxTree.Insert) val).getIndex()))
                .append("INSERT\n");
      } else if (val instanceof SyntaxTree.Set) {
        output.append(putVals(((SyntaxTree.Set) val).getList()))
                .append(putVals(((SyntaxTree.Set) val).getValue()))
                .append(putVals(((SyntaxTree.Set) val).getIndex()))
                .append("SET\n");
      } else if (val instanceof SyntaxTree.Get) {
        ...
        output.append(syntaxTreeToVMByteCode2(((SyntaxTree.PrintFunction) val).getProgram()));
      } else if (val instanceof SyntaxTree.ExitFunction) {
        output.append(syntaxTreeToVMByteCode2(((SyntaxTree.ExitFunction) val).getProgram()));
        ...
        SyntaxTree.CallFunction callInit = null;
        if (SyntaxTree.getClassesWithInit().contains(((SyntaxTree.CreateInstance) val).getClassName())) {
        ...
        output.append("MEMSIZE\nTOPTR\nMEMSIZE\nTOPTR\nPUT\tNUM0\nSTCKMOV\nPUT\tTXT").append(((SyntaxTree.CreateInstance) val).getClassName())
                .append("\nMEMPUT\n//PUT VARIABLES OF CLASS ").append(((SyntaxTree.CreateInstance) val).getClassName()).append("\n");
        if (callInit != null)
